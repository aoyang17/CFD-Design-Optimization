...
class DVGeometry_FFD_MODE(object):
    """
    ...
    def addGeoDVGlobal(self, dvName, value, func, lower=None, upper=None,
        ...
        value : float, or iterable list of floats
            ...
            parameter may be a single variable or a np array
    ...
    def addGeoDVLocal_Mode(self, dvName, phi, lower=None, upper=None, scale=1.0,
        ...
        self.DV_listLocal[dvName] = geoDVLocal_Mode(dvName, lower, upper, scale, axis, phi, config)
            
        return self.DV_listLocal[dvName].nVal

    def addGeoDVLocal_NNMode(self, dvName, ndv, nFFDpts, FFDmaxy, filename, lower=None, upper=None, scale=1.0,
                      axis='y', volList=None, pointSelect=None, config=None):
        """
        Add Modes of Local FFD DVs.
        These modes are combinations of local FFD DVs.
        ...
        self.DV_listLocal[dvName] = geoDVLocal_NNMode(dvName, lower, upper, scale, axis, ndv, nFFDpts, FFDmaxy, filename, config)
    ...
    def extractCoef(self, axisID):
        ...
        C = np.zeros((len(self.refAxis.topo.lIndex[axisNumber]), 3),
                        self.coef.dtype)
 
        C[:, 0] = np.take(self.coef[:, 0],
                             self.refAxis.topo.lIndex[axisNumber])
        C[:, 1] = np.take(self.coef[:, 1],
                             self.refAxis.topo.lIndex[axisNumber])
        C[:, 2] = np.take(self.coef[:, 2],
                             self.refAxis.topo.lIndex[axisNumber])
        ...
        for ipt in range(self.nPtAttach):
            ...
            if rotType == 0:
                ...
                if rotType == 7:
                    ...
                    deriv[0] = 0.0
                    deriv /= geo_utils.euclideanNorm(deriv) # Normalize
                    D = geo_utils.rotVbyW(D, deriv, np.pi/180*self.rot_theta[
                            self.curveIDNames[ipt]](self.links_s[ipt]))
    ...
    def convertDictToSensitivity(self, dIdxDict):
        ...
        dIdx = np.zeros(self.getNDV(), self.dtype)
        i = 0
        for key in self.DV_listGlobal:
            dv = self.DV_listGlobal[key]
...
# ----------------------------------------------------------------------
    ...
    def _update_deriv(self, iDV=0, h=1.0e-40j, oneoverh=1.0/1e-40, config=None, localDV=False):
        ...
        if len(self.axis) > 0:
            ...
            for iChild in range(len(self.children)):
                ...
                np.put(tmp[:, 0], self.ptAttachInd, 
                          np.imag(new_pts[:,0])*oneoverh)
                np.put(tmp[:, 1], self.ptAttachInd, 
                          np.imag(new_pts[:,1])*oneoverh)
                np.put(tmp[:, 2], self.ptAttachInd, 
                          np.imag(new_pts[:,2])*oneoverh)
            ...
            for key in self.DV_listGlobal:
                if self.DV_listGlobal[key].config is None or config in self.DV_listGlobal[key].config:
                    ...
                    for j in range(nVal):
                        ...
                        np.put(Jacobian[0::3, iDV], self.ptAttachInd, 
                                  deriv[0::3])
                        np.put(Jacobian[1::3, iDV], self.ptAttachInd, 
                                deriv[1::3])
                        np.put(Jacobian[2::3, iDV], self.ptAttachInd, 
                                  deriv[2::3])
    ...
    def _localDVJacobian(self, config=None):
        ...
        # LJC: things are a bit different since DVlocal_mode is treated as localDV
    ...
    def _cascadedDVJacobian(self,childDelta=True,config=None):
        ...
        if self.dXrefdXdvg is not None:
            for iDV in range(self.dXrefdXdvg.shape[1]):
                sum1 = np.sum(self.dXrefdXdvg[:, iDV])
                sum2 = np.sum(self.dCcdXdvg[:, iDV])
                if (sum1+sum2)==0:
                    continue
        ...
        if self.dXrefdXdvl is not None:  
            ...
            for iDV in range(self.dXrefdXdvl.shape[1]):
                ...
                sum1 = np.sum(self.dXrefdXdvl[:, iDV])
                sum2 = np.sum(self.dCcdXdvl[:, iDV])
...
class geoDVLocal_NNMode(object):
