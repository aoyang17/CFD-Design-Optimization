...
parser.add_argument("--output", type=str, default="output_Ninepts")
parser.add_argument("--opt", type=str, default="SLSQP", choices=["IPOPT", "SLSQP", "SNOPT"])
parser.add_argument("--gridFile", type=str, default="../input/ADODG4_Multiblock_Structured_Mesh_L3.cgns")
parser.add_argument("--FFDFile", type=str, default="../input/ADODG4_FFD.xyz")
...
aoa = [2.5, 3.1, 3.7, 2.13, 2.69, 3.24, 1.8, 2.31, 2.83]
mach =[0.82,0.82,0.82,0.85,0.85,0.85,0.88,0.88,0.88]
cl =[0.483,0.537,0.591,0.45,0.5,0.55,0.42,0.466,0.513]
w =[1.0/16.0,1.0/8.0,1.0/16.0,1.0/8.0,1.0/4.0,1.0/8.0,1.0/16.0,1.0/8.0,1.0/16.0]
rey = [4.82e6, 4.82e6, 4.82e6, 5e6, 5e6, 5e6, 5.18e6, 5.18e6, 5.18e6]
cmcon = 0.17
...
for i in range(nflowcase):
    ...
    ap.addDV("alpha", value=aoa[i], lower=0, upper=10.0, scale=1.0)
...
DVGeo.addGlobalDV(dvName="twist", value=[0] * nTwist, func=twist, lower=-1.0, upper=1.0, scale=1.0)
...
def objCon(funcs, printOK):
    ...
    for i in range(nflowcase):
        ...
        funcs["cl_con_" + ap.name] = funcs[ap["cl"]] - cl[i]
        if i == 4:
            funcs['cmy_con_'+ aeroProblems[i].name] = funcs[aeroProblems[i]["cmy"]] - cmcon
    if printOK:
...
optProb.addCon(f"cmy_con_{aeroProblems[4].name}", lower=0.0, upper=10.0, scale=10.0)
