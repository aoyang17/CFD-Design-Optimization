                ...
                if self.DV_listLocal[key].config is None or \
                   config in self.DV_listLocal[key].config:
                    ...
                    if self.DV_listLocal[key].localmode:
                        ...
                        for j in xrange(nVal):
                            for i in xrange(thisphi.shape[1]):
                                irow = i*3 + 1
...
class geoDVLocal_Mode(object):
    ...
    def __init__(self, dvName, lower, upper, scale, axis, phi,config):
        ...
        #self.coefList = numpy.zeros((npts, 2), 'intc')
    ...
    def __call__(self, coef, config):
        ...
        coefficients"""
        myvalue = numpy.zeros(self.phi.shape[1])
        for i in xrange(self.phi.shape[1]):
            for j in xrange(self.nVal):
...
class geoDVLocal_NNMode(object):
    ...
    def computeFFDs_real(self):
        ...
        if MPI.COMM_WORLD.rank == 0:
            ...
            scores = numpy.zeros(self.nFFDpts)
