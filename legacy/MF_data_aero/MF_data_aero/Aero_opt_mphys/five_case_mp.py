...
class Top(Multipoint):
    def setup(self):
        ...
        self.add_subsystem("lift", adflow_builder.get_post_coupling_subsystem())
        ...
        self.mphys_add_scenario("cruise2", ScenarioAerodynamic(aero_builder=adflow_builder))
        self.mphys_add_scenario("cruise3", ScenarioAerodynamic(aero_builder=adflow_builder))
        self.mphys_add_scenario("cruise4", ScenarioAerodynamic(aero_builder=adflow_builder))

        self.connect("mesh.x_aero0", "geometry.x_aero_in")
        self.connect("geometry.x_aero0", ["cruise0.x_aero", "cruise1.x_aero","cruise2.x_aero","cruise3.x_aero","cruise4.x_aero"])
    ...
    def configure(self):
        ...
        ap0 = AeroProblem(name="ap0", alpha=aoa, mach=mach[0], altitude=alt, reynolds=5e6, \
                reynoldsLength=1.0, T=326.45, areaRef= 3.407014, chordRef=1.0, \
                xRef=1.20777, yRef=0, zRef=.007669, evalFuncs=["cl", "cd"])        
        ap0.addDV("alpha", value=alpha[0], name="aoa0", units="deg")

        ap1 = AeroProblem(name="ap1", alpha=aoa, mach=mach[1], altitude=alt, reynolds=5e6, \
                reynoldsLength=1.0, T=326.45, areaRef= 3.407014, chordRef=1.0, \
                xRef=1.20777, yRef=0, zRef=.007669, evalFuncs=["cl", "cd"])        
        ap1.addDV("alpha", value=alpha[1], name="aoa1", units="deg")
        ...
        self.cruise2.coupling.mphys_set_ap(ap2)
        self.cruise2.aero_post.mphys_set_ap(ap2)
        self.cruise3.coupling.mphys_set_ap(ap3)
        self.cruise3.aero_post.mphys_set_ap(ap3)
        self.cruise4.coupling.mphys_set_ap(ap4)
        self.cruise4.aero_post.mphys_set_ap(ap4)
        ...
        # self.geometry.nom_addGeoDVGlobal(dvName="alpha", value=aoa, func=alpha)
        ...
        self.connect("aoa0", ["cruise0.coupling.aoa0", "cruise0.aero_post.aoa0"])
        self.connect("aoa1", ["cruise1.coupling.aoa1", "cruise1.aero_post.aoa1"])
        self.connect("aoa2", ["cruise2.coupling.aoa2", "cruise2.aero_post.aoa2"])
        self.connect("aoa3", ["cruise3.coupling.aoa3", "cruise3.aero_post.aoa3"])
        self.connect("aoa4", ["cruise4.coupling.aoa4", "cruise4.aero_post.aoa4"])
        ...
        self.add_design_var("local", lower=-0.5, upper=0.5, scaler=1.0)
        self.add_design_var("twist", lower=-10.0, upper=10.0, scaler=1.0)

        # add constraints and the objective
        self.add_constraint("cruise0.aero_post.cl", equals=cl[0], scaler=1.0)
        self.add_constraint("cruise1.aero_post.cl", equals=cl[1], scaler=1.0)
        self.add_constraint("cruise2.aero_post.cl", equals=cl[2], scaler=1.0)
        self.add_constraint("cruise3.aero_post.cl", equals=cl[3], scaler=1.0)
        self.add_constraint("cruise4.aero_post.cl", equals=cl[4], scaler=1.0)
        ...
        self.connect("cruise2.aero_post.cd", "drag.cd2")
        self.connect("cruise3.aero_post.cd", "drag.cd3")
        self.connect("cruise4.aero_post.cd", "drag.cd4")
...
prob.setup(mode="rev")
om.n2(prob, show_browser=False, outfile="aero_mpt_fivecase.html")
