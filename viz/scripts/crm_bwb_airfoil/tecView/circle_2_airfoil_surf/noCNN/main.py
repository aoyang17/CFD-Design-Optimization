...
ap.addDV('alpha', value=0.0, lower=0., upper=0.0, scale=1.0)
...
def cruiseFuncs(x):
    ...
    global iCFD
    DVGeo.writePlot3d('FFD'+str(iCFD)+'.xyz')
    if MPI.COMM_WORLD.rank == 0:
        ap = aeroProblems[0]
        f = open('converg.dat','a')
        if funcs['fail']:        
            f.write('%d %.15f %.1f\n'%(iCFD,funcs[ap['cd']],1.0))
        else:
            f.write('%d %.15f %.1f\n'%(iCFD,funcs[ap['cd']],0.0))
        f.close()
    iCFD=iCFD+1
